...
Parameters:
         diceCount - an integer specifying how many dice (1 or 2) to roll
Return Value: A pair of integers representing dice values; (d1, d2) for valid inputs
              or (0,0) if user skips.
Algorithm:
    ...
    2) Otherwise:
       ...
       d) Tokenize into integers:
          - If exactly diceCount tokens: check each is in [1..6].
            If valid, return it; if any 0 or out of range => error.
...
pair<int, int> Dice::rollCustom(int diceCount) const
{
    ...
    while (true)
    {
        ...
        // If user typed exactly diceCount tokens, validate them
